...
import com.ifeng.bigtrade.cms.entites.article.ModuleExample.Criteria;
import com.ifeng.bigtrade.cms.entites.article.ModuleExample.Criterion;
...
public class ModuleSqlProvider {
	...
	protected void applyWhere(SQL sql, ModuleExample example, boolean includeExamplePhrase) {
		if (example == null)
			return;
		...
		for (int i = 0; i < oredCriteria.size(); i++) {
			ModuleExample.Criteria criteria = (ModuleExample.Criteria) oredCriteria.get(i);
			if (criteria.isValid()) {
				if (firstCriteria) {
					firstCriteria = false;
				} else {
				...
				for (int j = 0; j < criterions.size(); j++) {
					ModuleExample.Criterion criterion = (ModuleExample.Criterion) criterions.get(j);
					if (firstCriterion) {
	...
	public String findAll() {
		SQL sql = new SQL();
		sql.SELECT(new String[] { "m.id", "m.name", "IFNULL(GROUP_CONCAT(t.`name`),'-1') tnames" });
		sql.FROM("cms_module m");
		sql.LEFT_OUTER_JOIN(
				new String[] { "cms_typemodule tm ON tm.moduleId = m.id", "cms_articletype t ON t.id=tm.typeId" });
		sql.GROUP_BY("m.id");
		return sql.toString();
	}
}